...
def targeted_attack_from_known_class(ace, interventions, inp, predicted_class, target_class, norm, budget):
    raise NotImplementedError("Targeted attack from known class not yet implemented!")

def untargeted_attack_from_known_class(ace, interventions, inp, predicted_class, norm, budget):
    raise NotImplementedError("Untargeted attack from known class not yet implemented!")

def targeted_attack_from_unknown_class(ace, interventions, inp, target_class, norm, budget):
    target_class = convert_one_hot_to_num(target_class)
    mask = torch.ones_like(inp)
    attack = inp.clone()

    while torch.norm(inp - attack, norm) < budget and torch.max(mask).item() == 1.0:  # TODO: fix later to break before we bust budget
...
def untargeted_attack_from_unknown_class(ace, interventions, inp, norm, budget):
    raise NotImplementedError("Untargeted attack from unkown class not yet implemented!")
...
def ace_attack(ace, interventions, inputs, predicted_classes=None, target_classes=None, norm=0, budget=100):
    ...
    for input_index in range(inputs_shape[0]):
        ...
        target_class = target_classes[input_index, :] if target_classes is not None else None

        if predicted_class is not None and target_class is not None:
            attack = targeted_attack_from_known_class(ace, interventions, inp, predicted_class, target_class, norm, budget)
        elif predicted_class is not None and target_class is None:
            attack = untargeted_attack_from_known_class(ace, interventions, inp, predicted_class, norm, budget)
        elif predicted_class is None and target_class is not None:
            attack = targeted_attack_from_unknown_class(ace, interventions, inp, target_class, norm, budget)
        else:
            attack = untargeted_attack_from_unknown_class(ace, interventions, inp, norm, budget)

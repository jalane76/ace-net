...
print(
    f"Jacobian at zeros equal: {np.allclose(pt_jacobian_at_zeros.numpy(), nd_jacobian_at_zeros)}"
)
print(
    f"Jacobian at ones equal: {np.allclose(pt_jacobian_at_ones.numpy(), nd_jacobian_at_ones)}"
)

pt_hessian_at_zeros = hessian(f(torch_zeros), torch_zeros)
pt_hessian_at_ones = hessian(f(torch_ones), torch_ones)
# print(
#    f"Hessian at zeros equal: {np.allclose(pt_hessian_at_zeros.numpy(), nd_hessian_at_zeros)}"
# )
# print(
#    f"Hessian at ones equal: {np.allclose(pt_hessian_at_ones.numpy(), nd_hessian_at_ones)}"
# )

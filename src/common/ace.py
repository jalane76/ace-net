...
def average_causal_effect(interventional_expectation):
    ...
    for x in range(size_x):
        for y in range(size_y):
            result[x, y, :] = interventional_expectation[x, y, :] - torch.mean(
                interventional_expectation[x, y, :]
            )
...
def interventional_expectation(
    ...
    output = model(
        mean
    ).detach()  # detach so we don't hold onto the computational graph through the whole ACE calculation causing a memory leak
    ...
    if method == "hessian_full":
        return __ie_hessian_full(
            model, mean, cov, interventions, result, progress=progress
        )
    elif method == "approximate":
        return __ie_approx(
            model, mean, cov, interventions, result, epsilon=epsilon, progress=progress
        )
    else:
        return __ie_hessian_diag(
            model, mean, cov, interventions, result, progress=progress
        )
...
def __ie_hessian_full(model, mean, cov, interventions, result, progress=False):
    ...
    with tqdm(total=size_x * size_y * size_i, disable=not progress) as pbar:
        for x in range(size_x):
            cov_row = cov[
                x, :
            ].clone()  # hold out the covariance row we'll intervene upon
            cov_col = cov[
                :, x
            ].clone()  # hold out the covariance col we'll intervene upon
            ...
            for i in range(size_i):
                inp = (
                    mean.clone().detach()
                )  # clone so we don't change the original mean
                ...
                inp = inp.reshape(
                    inp_shape
                )  # reshape back to original shape for input to model
                ...
                for y in range(size_y):
                    ...
                    result[x, y, i] = output.reshape(-1)[y] + 0.5 * torch.trace(
                        torch.matmul(h[0, y], cov)
                    )
                    pbar.update(1)
            ...
            for x in range(size_x):
                for i in range(size_i):
                    ...
                    for xx in range(size_x):
                        ...
                        cov_val = cov[
                            xx, x
                        ].clone()  # hold out the covariance value we'll intervene upon
...
def __ie_approx(
    model, mean, cov, interventions, result, epsilon=0.000001, progress=False
):
